...
**Вне скоупа MVP:** пользователи и авторизация (кроме простого токена), комментарии, вложения, уведомления, мобильное приложение, offline-синхронизация между устройствами, спринты/оценки.
...
| Безопасность | Для self-hosted-режима — статический bearer-токен в конфиге; HTTPS на стороне reverse-proxy |
...
Базовый префикс: `/api/v1`. Аутентификация: `Authorization: Bearer <token>` (в локальном десктоп-режиме отключаема). Ошибки — единый формат:
