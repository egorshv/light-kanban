...
def create_task(body: TaskCreate, conn=Depends(get_conn)):
...
def get_task(task_id: str, conn=Depends(get_conn)):
    return tasks_service.get_task_with_links(conn, task_id)


@router.patch("/tasks/{task_id}", response_model=TaskOut)
def update_task(task_id: str, body: TaskPatch, conn=Depends(get_conn)):
    return tasks_service.update_task(conn, task_id, body.model_dump(exclude_unset=True))


@router.post("/tasks/{task_id}/move", response_model=TaskWithWarning)
def move_task(task_id: str, body: TaskMove, conn=Depends(get_conn)):
    task, warning = tasks_service.move_task(conn, task_id, body.column_id, body.position)
    return {**task, "wip_warning": warning}


@router.delete("/tasks/{task_id}", status_code=204)
def delete_task(task_id: str, conn=Depends(get_conn)):
    tasks_service.delete_task(conn, task_id)
...
def search_tasks(
    ...
    return tasks_service.search_tasks(conn, board_id, q, priority, due_before)
